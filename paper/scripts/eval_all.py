...
def evaluate(args, load_args=False):
    ...
    for loss_name, loss_type_dict in loss_dict_all.items():
        ...
        for loss_type_name, loss_list_all in loss_type_dict.items():
            if len(loss_list_all) > 0:
                ...
                loss_name_full = f'{loss_name}'

                plot_loss_avg_dict[loss_name_full] = loss_avg
                plot_loss_std_dict[loss_name_full] = loss_std
    ...
    plot_eval_loss(f'{loss_name_full} loss', plot_loss_avg_dict, loss_std_dict=plot_loss_std_dict,
        path=os.path.join('paper', 'images', f'{dataset}_loss.png'))
