...
class PrecodedPlanner(Planner):
    ...
    def eval_soln(self, param_seq, state_cur, state_end, target_shape):
        ...
        state_end_norm, state_goal_norm = normalize_state(self.args, state_end[0], target_shape['surf'])
        ...
        pkl_path = os.path.join(self.rollout_path, 'anim_args', f'{self.args.env}_anim_args.pkl')
        with open(pkl_path, 'wb') as f:
            pickle.dump({'args': self.args, 'row_titles': [title], 'state_seqs': [state_seq_aug], 'target': target_shape['surf']}, f)
...
class CutterPlanarPlanner(PrecodedPlanner):
    ...
    def plan(self, state_cur, target_shape, rollout_path, max_n_actions, rs_loss_threshold=float('inf')):
        ...
        state_cur_pcd.points = o3d.utility.Vector3dVector(state_cur.squeeze().cpu().numpy())
        ...
        param_seq = torch.tensor([x_center, cut_y - 0.008, np.pi / 4], dtype=torch.float32).unsqueeze(0)
    ...
    def eval_soln(self, param_seq, state_cur, target_shape):
        ...
        state_cur_pcd_crop = copy.deepcopy(state_cur_pcd).crop(crop_bbox)
        cropped_mesh = alpha_shape_mesh_reconstruct(state_cur_pcd_crop, alpha=0.2, mesh_fix=False)
        if self.args.surface_sample:
            state_end_pcd = o3d.geometry.TriangleMesh.sample_points_poisson_disk(cropped_mesh, state_cur.squeeze().shape[0])
        else:
            raise NotImplementedError
        state_end = np.asarray(state_end_pcd.points)[None, :, :]

        return super().eval_soln(param_seq, state_cur.cpu().numpy(), state_end, target_shape)
...
class CutterCircularPlanner(PrecodedPlanner):
    ...
    def eval_soln(self, param_seq, state_cur, target_shape):
        ...
        return super().eval_soln(param_seq.squeeze(), state_cur_numpy, state_cur_numpy, target_shape)
...
class PusherPlanner(PrecodedPlanner):
    ...
    def eval_soln(self, param_seq, state_cur, target_shape):
        ...
        state_cur_crop_idx = np.linalg.norm(state_cur_numpy[:, :2] - 
            np.tile(param_seq.cpu().numpy()[:2], (state_cur_numpy.shape[0], 1)), axis=1) < r
        ...
        cropped_mesh = alpha_shape_mesh_reconstruct(state_cur_pcd_crop, alpha=0.2, mesh_fix=False)
        if self.args.surface_sample:
            state_end_pcd = o3d.geometry.TriangleMesh.sample_points_poisson_disk(cropped_mesh, state_cur_numpy.shape[0])
        else:
            raise NotImplementedError
        state_end = np.asarray(state_end_pcd.points)[None, :, :]
...
class SpatulaPlanner(PrecodedPlanner):
    ...
    def plan(self, state_cur, target_shape, rollout_path, max_n_actions, rs_loss_threshold=float('inf')):
        ...
        if 'small' in self.args.env:
            return torch.tensor([self.center[0].item(), self.center[1].item(), 0.395, -0.29], dtype=torch.float32).unsqueeze(0)
        else:
            return torch.tensor([0.5, -0.3, 0.395, -0.29], dtype=torch.float32).unsqueeze(0)

    def eval_soln(self, param_seq, state_cur, target_shape):
        param_seq = param_seq.squeeze()
        state_cur_numpy = state_cur.squeeze().cpu().numpy()
        if 'small' in self.args.env:
            state_trans = np.array([param_seq[2], param_seq[3], 0.025]) - self.center.cpu().numpy()
            state_end = state_cur_numpy + np.tile(state_trans, (state_cur_numpy.shape[0], 1))
...
class HookPlanner(PrecodedPlanner):
    ...
    def eval_soln(self, param_seq, state_cur, target_shape):
        return super().eval_soln(param_seq.squeeze(), state_cur.cpu().numpy(), target_shape['surf'][None, :, :], target_shape)