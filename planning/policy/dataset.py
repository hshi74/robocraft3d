    ...
    def _get_data(self):
        ...
        for i in range(len(seq_path_list)):
            ...
            for j in range(len(sample_path_list)):
                ...
                for k in range(len(state_path_list) - self.args.n_actions):
                    ...
                    state_out = load_data(self.data_names, state_path_list[k+self.args.n_actions])[0]
                    ...
                    if self.args.early_fusion:
                        state_in = np.concatenate((state_in, np.full((state_in.shape[0], 1), 0)), axis=1)
                        state_out = np.concatenate((state_out, np.full((state_out.shape[0], 1), 1)), axis=1)
                    ...
                    for ii in range(params.shape[0]):
                        if ii % (params.shape[0] // self.args.n_actions) != self.args.rot_idx:
                            params[ii] /= self.args.scale_ratio
                    param_seq_list.append(params)

        param_seq = np.stack(param_seq_list).astype(np.float32)
        param_seq_norm = param_seq # (param_seq - self.args.stats['mean']) / self.args.stats['std']
        ...
        for i in range(self.args.n_bins.shape[0]):
            if i % (self.args.n_bins.shape[0] // self.args.n_actions) == self.args.rot_idx:
                ...
                bin_diff = -np.cos((np.tile(param_seq_norm[:, i:i+1], (1, self.args.n_bins[i])) - self.args.bin_centers[i]) * period_ratio)
            else:
                bin_diff = np.abs(np.tile(param_seq_norm[:, i:i+1], (1, self.args.n_bins[i])) - self.args.bin_centers[i])
            bin_idx_list = [np.argsort(bin_diff[j])[:2] for j in range(param_seq_norm.shape[0])]
        ...
        point_set = []
        for state_concat, bin_idx, params_norm in zip(state_concat_list, bin_ind, param_seq_norm):
