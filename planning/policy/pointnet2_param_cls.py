...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        if args.early_fusion:
            ...
            self.sa1_in = PointNetSetAbstractionMsg(512, [0.1, 0.2, 0.4], [16, 32, 128], in_channel,[[32, 32, 64], [64, 64, 128], [64, 96, 128]])
            self.sa2_in = PointNetSetAbstractionMsg(128, [0.2, 0.4, 0.8], [32, 64, 128], 320,[[64, 64, 128], [128, 128, 256], [128, 128, 256]])
            self.sa3_in = PointNetSetAbstraction(None, None, None, 640 + 3, [256, 512, 1024], True)
            
            self.sa1_out = PointNetSetAbstractionMsg(512, [0.1, 0.2, 0.4], [16, 32, 128], in_channel,[[32, 32, 64], [64, 64, 128], [64, 96, 128]])
            self.sa2_out = PointNetSetAbstractionMsg(128, [0.2, 0.4, 0.8], [32, 64, 128], 320,[[64, 64, 128], [128, 128, 256], [128, 128, 256]])
            self.sa3_out = PointNetSetAbstraction(None, None, None, 640 + 3, [256, 512, 1024], True)
    ...
    def forward(self, xyz):
        ...
        for i in range(len(self.args.n_bins)):
            ...
            if i % (self.args.n_bins.shape[0] // self.args.n_actions) == self.args.rot_idx:
                # sin and cos
                pred_offset = pred_offset.view([B, -1, 2])
                pred_offset_norm = F.normalize(pred_offset, dim=2)
                pred_angle = torch.atan2(pred_offset_norm[:, :, 1], pred_offset_norm[:, :, 0])
                half_bin_size = torch.tensor((self.args.tool_params['rot_range'][1] - self.args.tool_params['rot_range'][0]) / \
                    self.args.n_bins[i], device=self.args.device)
...
class get_loss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        reg_loss = 0.0
        for i in range(len(self.args.n_bins)):
            cls_loss += self.args.cls_weight * self.cls_loss_func(pred_ind[i], target_idx[:, i, 0])
            for j in range(n_gt_bins):
                ...
                pred_offset = pred_offsets[i][torch.arange(bin_idx_gt.shape[0]), bin_idx_gt]
                if i % (self.args.n_bins.shape[0] // self.args.n_actions) == self.args.rot_idx:
                    period_ratio = np.pi * 2 / self.args.tool_params['rot_scope']
                    reg_loss += -self.args.orient_weight / n_gt_bins * torch.cos((target_params[:, i] - \
                        bin_centers_batch - pred_offset) * period_ratio).mean()
                else:
                    reg_loss += 1 / n_gt_bins * self.reg_loss_func(bin_centers_batch + pred_offset, target_params[:, i])
