...
def main():
    ...
    inspect_dir = f'{root}/data/planning/inspect/{date}'
    os.system(f"mkdir -p {inspect_dir}/anim")
    dump_dir_list = []
    for tool_type in tool_arr:
        dy_dir = f"{root}/data/planning/data_{tool_type}_robot_v4_surf_nocorr_full_normal_keyframe=16_action=2_p=300/dump/"
    ...
    for dump_dir in dump_dir_list:
        ...
        for dataset in dataset_list:
            for i in range(n_rollout):
                ...
                if os.path.exists(res_anim):
                    shutil.copyfile(res_anim, f'{inspect_dir}/anim/{test_name}_{dataset}_{i}_{str(0).zfill(3)}.mp4')
        ...
        with open(f'{inspect_dir}/eval_{test_name}.txt', 'r') as log_file:
            ...
            while i < len(log):
                ...
                if 'train phase' in line: row['dataset'] = 'train'
                if 'valid phase' in line: row['dataset'] = 'valid'
                if 'test phase' in line: row['dataset'] = 'test'
