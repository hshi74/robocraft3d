...
from utils.pcd_aug_provider import *
...
def get_test_name(args):
    ...
    if args.early_fusion: test_name.append(f'ef=1')
    if args.use_normals: test_name.append(f'normal=1')
...
def rollout(args, dy_args, gnn, planner, state_in_ori, state_out_ori):
    ...
    if args.early_fusion:
        point_labels = torch.cat((torch.zeros((*state_in.shape[:-1], 1), device=device), 
            torch.ones((*state_out.shape[:-1], 1), device=device)), dim=1)
    ...
    pred_params_torch = torch.tensor(pred_params, dtype=torch.float32).reshape((args.n_actions, -1))
    center = torch.mean(state_cur.squeeze(), dim=0).cpu()
    min_bounds = torch.min(state_cur, dim=1).values.squeeze().cpu().numpy()
    init_pose_seq = params_to_init_pose(dy_args, center, args.tool_params, pred_params_torch).numpy()
    act_seq_dense = params_to_actions(dy_args, args.tool_params, pred_params_torch, min_bounds, step=1).numpy()
    act_seq = params_to_actions(dy_args, args.tool_params, pred_params_torch, min_bounds, step=dy_args.time_step).numpy()

    with torch.no_grad():
        state_pred_seq, _, _ = gnn.rollout(copy.deepcopy(state_cur), init_pose_seq[None], act_seq_dense[None])

    state_cur_wshape = np.concatenate((state_cur.cpu().numpy()[0, :, :3], dy_args.floor_state, init_pose_seq[0]), axis=0)
    state_pred_seq_wshape = add_shape_to_seq(dy_args, state_pred_seq.cpu().numpy()[0], init_pose_seq, act_seq)
    state_pred_seq_wshape = np.concatenate((state_cur_wshape[None], state_pred_seq_wshape), axis=0)
        ...
        for i in range(len(seq_path_list)):
            ...
            for j in range(len(sample_path_list)):
                ...
                for k in range(len(state_path_list)):
                    ...
                    state_torch = torch.tensor(state[None, :dy_args.n_particles], dtype=torch.float32, device=device)
                ...
                for k in range(len(state_list) - args.n_actions):
                    if k == 0:
                        ...
                        state_in_normals = get_normals(state_out_pred, pkg='torch').to(device)
                    ...
                    if state_out.shape[-1] < 6:
                        state_out_normals = get_normals(state_out, pkg='torch').to(device)
                    ...
                    for ii in range(gt_params.shape[0]):
                        if ii % (gt_params.shape[0] // args.n_actions) == args.rot_idx:
                            ...
                            bin_diff = -np.cos((gt_params[ii] - args.bin_centers[ii]) * period_ratio)
                        else:
                            bin_diff = np.abs(gt_params[ii] / args.scale_ratio - args.bin_centers[ii])
                        gt_ind.append(np.argsort(bin_diff)[0])

                    # import pdb; pdb.set_trace()
                    state_out_target = copy.deepcopy(state_out[:, :, :3])
                    pred_idx, pred_params, state_pred_seq_wshape = rollout(args, dy_args, gnn, planner, state_in, state_out)
                    state_out_pred = torch.tensor(state_pred_seq_wshape[-1:, :dy_args.n_particles], dtype=torch.float32, device=device)

                    state_pred_norm = state_out_pred - torch.mean(state_out_pred, dim=1)
                    state_target_norm = state_out_target - torch.mean(state_out_target, dim=1)
                    chamfer_loss = chamfer(state_pred_norm[0], state_target_norm[0], pkg='torch').item()

                    state_vis_dict[f'pred_{k+1}: {pred_idx}, {[round(x, 3) for x in pred_params]}'] = [state_out_pred[0].cpu()]
                    state_vis_dict[f'gt_{k+1}: {gt_ind}, {[round(x, 3) for x in gt_params]}'] = [state_out_target[0].cpu()]
                ...
                if visualize:
                    render_frames(dy_args, list(state_vis_dict.keys()), list(state_vis_dict.values()), res='low', axis_off=False, focus=False,
                        path=os.path.join(dump_path, 'eval'), name=f'{phase}_{str(i).zfill(3)}_{str(j).zfill(3)}.png')

                    render_anim(dy_args, [f'GNN'], [np.concatenate(state_pred_seq_list, axis=0)], res='low', 
                        path=os.path.join(dump_path, 'eval', f'{phase}_{str(i).zfill(3)}_{str(j).zfill(3)}.mp4'))
...
def compute_stats(args):
    ...
    for i in range(param_seq.shape[1]):
        if i % (param_seq.shape[1] // args.n_actions) == args.rot_idx:
            half_bin_size = (args.tool_params['rot_range'][1] - args.tool_params['rot_range'][0]) / args.n_bins[i]
            bin_centers.append(args.tool_params['rot_range'][0] + half_bin_size * np.arange(args.n_bins[i]))
        else:
            half_bin_size = (param_seq_max[i] - param_seq_min[i]) / (args.n_bins[i] - 1)
            bin_centers.append(param_seq_min[i] + half_bin_size * np.arange(args.n_bins[i]))
...
def train(args, dump_path):
    ...
    datasets = {phase: PolicyDataset(args, phase, rollout_ratio=r) for phase, r in phases.items()}

    dataloaders = {phase: DataLoader(datasets[phase], batch_size=args.batch_size, 
        shuffle=True if phase == 'train' else False,
        num_workers=args.num_workers) for phase in phases.keys()}
    ...
    for epoch in range(args.n_epoch):
        for phase in phases:
            with torch.set_grad_enabled(phase=='train'):
                ...
                reg_loss_list = []
                loss_list = []
                for i, (points, target_idx, target_params) in enumerate(tqdm(dataloaders[phase], desc=f'epoch {epoch}/{args.n_epoch}')):
                    # import pdb; pdb.set_trace()
                    if phase == 'train':
                        # data augmentation
                        points = points.numpy()

                        points[:, :args.n_particles] = shuffle_points(points[:, :args.n_particles])
                        points[:, args.n_particles:] = shuffle_points(points[:, args.n_particles:])
                        points[:, :args.n_particles] = random_point_dropout(points[:, :args.n_particles], max_dropout_ratio=0.4)
                        points[:, args.n_particles:] = random_point_dropout(points[:, args.n_particles:], max_dropout_ratio=0.4)
                        ...
                        points, target_params, target_idx = random_rotate_z_point_cloud_with_normal(args, points, target_params, target_idx)

                        points[:, :args.n_particles, :3] = shift_point_cloud(points[:, :args.n_particles, :3])
                        points[:, args.n_particles:, :3] = shift_point_cloud(points[:, args.n_particles:, :3])
                    ...
                    cls_loss, reg_loss = criterion(pred, (target_idx.long(), target_params))
                ...
                print(f'lr: {get_lr(optimizer):.6f}; {phase} loss: {loss_avg.item():.6f}; '
...
def main():
    ...
    args.plan_dataf = f'data/planning/data_{args.plan_tool}_robot_v4_surf_nocorr_full_normal_keyframe=16_action=2_p={args.n_particles}'
    args.device = device
    ...
    args.bin_centers_torch = [torch.tensor(x, dtype=torch.float32, device=device) for x in args.bin_centers]
