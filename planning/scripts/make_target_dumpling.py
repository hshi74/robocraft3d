...
target_root = '/scr/hshi74/projects/robocook/target_shapes/multi_tool/'
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

def make_target(args, target_name, step, state_init, tool_name, tool_model_path, params):
    print(target_name)
    ...
    letter_root = os.path.join(target_root, target_name, f'{str(step).zfill(3)}_{tool_name}')
    ...
    with torch.no_grad():
        state_seq, _, _ = gnn.rollout(copy.deepcopy(state_cur), init_pose_seq.unsqueeze(0), act_seq.unsqueeze(0))
    ...
    state_data = [state_final_numpy[:args.n_particles], shape_quats, tool_args.scene_params]
    store_data(tool_args.data_names, state_data, os.path.join(letter_root, f'{str(step).zfill(3)}_surf.h5'))
    ...
    render_frames(tool_args, titles, [state_init[None], state_final_numpy[None]], res='low', axis_off=False, focus=False,
        path=letter_root, name=f'{str(step).zfill(3)}_sampled.png')

    act_seq_sparse = params_to_actions(tool_args, tool_params, param_seq, min_bounds, step=tool_args.time_step)
    state_seq_wshape = add_shape_to_seq(tool_args, state_seq.squeeze().cpu().numpy(), init_pose_seq.cpu().numpy(), act_seq_sparse.cpu().numpy())
...
def main():
    ...
    target_dict = {
        'dumpling_gnn': {
            ...
            'params': [[[0.0, 0.785, 0.015], [0.0, 2.355, 0.015]], [[0.0, 0.0, 0.01, 0.0]], 
                [[-0.02, 0, 0.012, 0], [0, -0.02, 0.007, 1.57], [0.02, 0.0, 0.007, -1.57], [0.0, 0.02, 0.007, 3.14]]]
        }
    }

    for target_name, info in target_dict.items():
        # os.system(f'cp -r /scr/hshi74/projects/robocook/target_shapes/alphabet_real/{target_name}/000 ' + \
        #     f'/scr/hshi74/projects/robocook/target_shapes/alphabet_gnn/{target_name}/000')

        for i in range(len(info['tool'])):
            if i == 0:
                state_path = os.path.join(target_root.replace('gnn', 'real'), target_name, f'{str(i+1).zfill(3)}_cutter_planar/{str(i+1).zfill(3)}_surf.h5')
            else:
                state_path = os.path.join(target_root.replace('gnn', 'real'), target_name, f"{str(i+1).zfill(3)}_{info['tool'][i-1]}/{str(i+1).zfill(3)}_surf.h5")

            state_cur = load_data(args.data_names, state_path)[0]
            make_target(args, target_name, i+2, state_cur, info['tool'][i], tool_model_dict[info['tool'][i]], info['params'][i])
