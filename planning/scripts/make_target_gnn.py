...
target_root = '/scr/hshi74/projects/robocook/target_shapes/alphabet_gnn/'
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

def make_target(args, letter_name, step, state_init, tool_name, tool_model_path, params):
    print(letter_name)
    ...
    letter_root = os.path.join(target_root, letter_name, f'{str(step).zfill(3)}_{tool_name}')
    ...
    os.system(f"cp -r {letter_root.replace('gnn', 'real')}/{str(step).zfill(3)}_raw.ply " + \
...
def main():
    ...
    for letter_name, info in letters_dict.items():
        # os.system(f'cp -r /scr/hshi74/projects/robocook/target_shapes/alphabet_real/{letter_name}/000 ' + \
        #     f'/scr/hshi74/projects/robocook/target_shapes/alphabet_gnn/{letter_name}/000')

        for i in range(len(info['tool'])):
            if i == 0:
                state_path = os.path.join(target_root.replace('gnn', 'real'), letter_name, f'{str(i).zfill(3)}/{str(i).zfill(3)}.h5')
            else:
                state_path = os.path.join(target_root.replace('gnn', 'real'), letter_name, f"{str(i).zfill(3)}_{info['tool'][i-1]}/{str(i).zfill(3)}.h5")

            state_cur = load_data(args.data_names, state_path)[0]
            make_target(args, letter_name, i+1, state_cur, info['tool'][i], tool_model_dict[info['tool'][i]], info['params'][i])
