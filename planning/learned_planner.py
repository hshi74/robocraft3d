...
class LearnedPlanner(Planner):
    def __init__(self, args, name, tool_model_path_list, tool_params):
        ...
        for tool_model_path in tool_model_path_list:
            ...
            tool_args_dict = np.load(f'{tool_model_path}_args.npy', allow_pickle=True).item()
            ...
            pretrained_dict = torch.load(f'{tool_model_path}.pth', map_location=self.device)
            planner.load_state_dict(pretrained_dict, strict=False)
            planner.eval()
            self.planner_dict['model'].append(planner)

        dy_args_dict = np.load(f'{tool_args.plan_dataf}/args.npy', allow_pickle=True).item()
        self.dy_args = copy.deepcopy(tool_args)
        self.dy_args.__dict__ = dy_args_dict
        self.dy_args.ee_fingertip_T_mat = np.array([[0.707, 0.707, 0, 0], 
            [-0.707, 0.707, 0, 0], [0, 0, 1, 0.1034], [0, 0, 0, 1]])
    ...
    def plan(self, state_cur, target_shape, rollout_path, max_n_actions, rs_loss_threshold=float('inf')):
        ...
        self.param_bounds = get_param_bounds(self.dy_args, self.tool_params, min_bounds, max_bounds)

        best_loss = float('inf')
        for i in range(max_n_actions):
            pred_idx, param_seq, loss, state_seq = self.take_initial_guess(i, state_cur, target_shape)
            if self.optim_algo == 'RS':
                param_seq_opt = param_seq
                loss_opt = loss
            elif self.optim_algo == 'CEM':
                param_seq_CEM, loss_seq_CEM = self.optimize_CEM((param_seq, loss, state_seq), state_cur, target_shape)
                param_seq_opt = param_seq_CEM
                loss_opt = loss_seq_CEM
            elif self.optim_algo == 'GD':
                with torch.set_grad_enabled(True):
                    param_seq_GD, loss_seq_GD = self.optimize_GD(param_seq, state_cur, target_shape)
    ...
    def eval_soln(self, param_seq, state_cur, target_shape):
        min_bounds = torch.min(state_cur, dim=1).values.squeeze().cpu().numpy()
        init_pose_seq = params_to_init_pose(self.dy_args, self.center, self.tool_params, param_seq).numpy()
        act_seq_dense = params_to_actions(self.dy_args, self.tool_params, param_seq, min_bounds, step=1).numpy()
        act_seq = params_to_actions(self.dy_args, self.tool_params, param_seq, min_bounds, step=self.dy_args.time_step).numpy()

        with torch.no_grad():
            state_pred_seq, _, _ = self.gnn.rollout(copy.deepcopy(state_cur), init_pose_seq[None], act_seq_dense[None])

        state_cur_wshape = np.concatenate((state_cur.cpu().numpy()[0, :, :3], self.dy_args.floor_state, init_pose_seq[0]), axis=0)
        state_seq_wshape = add_shape_to_seq(self.dy_args, state_pred_seq.cpu().numpy()[0], init_pose_seq, act_seq)
        state_seq_wshape = np.concatenate((state_cur_wshape[None], state_seq_wshape), axis=0)

        state_out_pred = torch.tensor(state_seq_wshape[-1, :self.dy_args.n_particles], device=self.device)
        ...
        with open(pkl_path, 'wb') as f:
            pickle.dump({'args': self.dy_args, 'row_titles': [title], 'state_seqs': [state_seq_wshape], 'target': target_shape['surf']}, f)
        ...
        with open(os.path.join(self.rollout_path, 'param_seqs', f'{self.dy_args.env}_param_seq.yml'), 'w') as f:
            yaml.dump({self.dy_args.env: param_seq.tolist()}, f, default_flow_style=True)
    ...
    def take_initial_guess(self, idx, state_cur, target_shape):
        ...
        state_out_ori = torch.tensor(target_shape['surf'], device=self.device).unsqueeze(0)
        ...
        if args.early_fusion:
            point_labels = torch.cat((torch.zeros((*state_in.shape[:-1], 1), device=self.device), 
                torch.ones((*state_out.shape[:-1], 1), device=self.device)), dim=1)
        ...
        pred_params_tensor = torch.tensor(pred_params, dtype=torch.float32).reshape((args.n_actions, -1))
        ...
        loss, state_seq = self.rollout(pred_params_tensor.unsqueeze(0), state_cur, target_shape)
    ...
    def rollout(self, param_seqs, state_cur, target_shape):
        ...
        for param_seq in param_seqs:
            ...
            act_seq = params_to_actions(self.dy_args, self.tool_params, param_seq, min_bounds)
        ...
        for i in range(n_batch):
            ...
            state_seq, _, _ = self.gnn.rollout(state_cur, init_pose_seqs[start: end], act_seqs[start: end])
    ...
    def evaluate_traj(
        ...
        for i in range(state_seqs.shape[0]):
            ...
            state_goal = torch.tensor(target_shape['surf'], device=self.device, dtype=torch.float32).unsqueeze(0)
    ...
    def optimize_CEM(    
        ...
        best_param_seq, best_loss, best_state_seq = param_seq_cur, loss_cur, state_seq_cur
        ...
        for j in range(self.CEM_sample_iter):
            ...
            if plateau_iter >= plateau_max_iter:
                interrupt = f"---> Stop criterion 2.b: improving less than {plateau_threshold} " + \
        ...
        plot_eval_loss('Control CEM Loss', traj_loss_dict, 
            path=os.path.join(self.rollout_path, 'optim_plots', f"{self.dy_args.env}_CEM_loss"))
    ...
    def optimize_GD(
        ...
        plot_eval_loss('Control GD Loss', loss_list_dict, 
            path=os.path.join(self.rollout_path, 'optim_plots', f"{self.dy_args.env}_GD_loss"))
    ...
    def render_selections(self, name, state_cur, param_seq_opt, loss_opt, state_seq_opt, target_shape):
        ...
        for i in range(len(param_seq_opt)):
            ...
            act_seq = params_to_actions(self.dy_args, self.tool_params, param_seq_opt[i], min_bounds, step=self.dy_args.time_step)
            state_seq_wshape = add_shape_to_seq(self.dy_args, state_seq_opt[i].cpu().numpy(), 
        ...
        with open(pkl_path, 'wb') as f:
            pickle.dump({'args': self.dy_args, 'row_titles': row_titles, 'state_seqs': state_seq_wshape_list, 
